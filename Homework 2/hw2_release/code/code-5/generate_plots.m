...
errors_xval_kern = zeros(100,size(N_folds,2)); % errors_xval(i,j) records the [N_folds(j)]-fold cross validation error in trial i 
errors_test_kern = zeros(100,size(N_folds,2)); % errors_xval(i,j) records the true test error in trial i (the entire row will be identical)
K = 1;
sigma = 1;
distfunc = 'l2';
for trial = 1:100
    ...
    for i = 1:numel(N_folds)
        ...
        % run the cross-validation error function
        % errors_xval(trial,i) = knn_xval_error(X_train, Y_train, K, part, distfunc);
        errors_xval_kern(trial,i) = kernreg_xval_error(X_train, Y_train, sigma, part, distfunc);
        
        % run kNN/kernel-regression on all training data to compute test error
        % [labels] = kernel_regression(X_train, Y_train, X_test, K, distfunc);
        [labels] = kernel_regression(X_train, Y_train, X_test, sigma);
        
        errors_test_kern(trial,i) = sum(labels ~= Y_true)/size(labels,1);
...
figure;
y = mean(errors_xval_kern); e = std(errors_xval_kern); x = [3, 5, 10, 15]; % <- computes mean across all trials
errorbar(x, y, e);
hold on;
y = mean(errors_test_kern); e = std(errors_test_kern); x = [3, 5, 10, 15]; % <- computes mean across all trials
errorbar(x, y, e);
title('Original data, N = [3, 5, 10, 15]');
xlabel('N');
ylabel('Error');

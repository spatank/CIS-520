function labels = logistic_regression(Xtrain,Ytrain,Xtest,stepsize,iterations)
    ...
    % You may use gradient ascent as a subroutine within this function to
    % make things simpler. Use the version of gradient ascent
    ...
    % weights_decay = gradient_ascent_decay(Xtrain, Ytrain, stepsize, iterations);
    weights_fixed = gradient_ascent_fixed(Xtrain, Ytrain, stepsize, iterations);
    
    labels = sign(Xtest*weights_fixed);
    labels(labels < 0) = 0;
    labels(labels > 0) = 1;
